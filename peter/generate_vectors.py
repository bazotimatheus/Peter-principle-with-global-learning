...
def update_skill_vector(dataset, qualifications, learning_coeficient):
  for skill in range(0, COMPETENCES):
    levels = dataset['level'].values - 1
    skills = dataset['skill_' + str(skill)].values
    
    new_skills = []
    for level, current_skill in zip(levels, skills):
      new_skill = current_skill + learning_coeficient * qualifications[level, skill]
